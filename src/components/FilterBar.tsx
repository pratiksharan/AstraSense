...
const FilterBar = ({ activeFilter, onFilterChange, sortBy, onSortChange }: FilterBarProps) => {
  ...
  return (
    <div className="mx-4 sm:mx-6 mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="overflow-x-auto pb-1 -mb-1 sm:overflow-visible sm:pb-0 sm:mb-0">
        <div className="flex items-center gap-1.5 min-w-max sm:min-w-0 sm:flex-wrap pr-1">
          {typeFilters.map(tag => (
            <button
              key={tag}
      ...
      <select
        ...
        className="w-full sm:w-auto sm:max-w-xs px-3 py-1.5 rounded-lg bg-secondary border border-border text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer"
      >
        {sortOptions.map(o => (
